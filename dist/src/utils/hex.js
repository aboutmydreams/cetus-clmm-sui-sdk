...
export function shortString(str, start = 4, end = 4) {
    const slen = Math.max(start, 1);
    const elen = Math.max(end, 1);
    return `${str.slice(0, slen + 2)} ... ${str.slice(-elen)}`;
}
export function shortAddress(address, start = 4, end = 4) {
    return shortString(addHexPrefix(address), start, end);
}
export function checkAddress(address, options = { leadingZero: true }) {
 ...
 * @param binaryData
 */
export function hexToNumber(binaryData) {

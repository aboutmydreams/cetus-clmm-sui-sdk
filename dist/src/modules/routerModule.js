...
import { ClmmExpectSwapModule, ClmmIntegrateRouterModule } from '../types/sui';
...
export class RouterModule {
    ...
    async price(from, to, amount, byAmountIn, priceSlippagePoint, partner, swapWithMultiPoolParams) {
        ...
        const poolAddress = preRouterSwapParams[bestIndex].poolBC != null
    ...
    async preRouterSwapA2B2C(params) {
        ...
        const valueData = simulateRes.events?.filter((item) => {
            return (extractStructTagFromType(item.type).name === `CalculatedRouterSwapResultEvent` ||
        ...
        for (let i = 0; i < valueData.length; i += 1) {
            ...
            if (params[0].byAmountIn) {
                ...
                const amount = params[i].stepNums > 1
        ...
        if (params[tempIndex].stepNums > 1) {
            targetSqrtPrice.push(valueData[tempIndex].parsedJson.data.target_sqrt_price_ab, valueData[tempIndex].parsedJson.data.target_sqrt_price_cd);
            currentSqrtPrice.push(valueData[tempIndex].parsedJson.data.current_sqrt_price_ab, valueData[tempIndex].parsedJson.data.current_sqrt_price_cd);
        }
        else {
    ...
    async getPoolWithTVL() {
        ...
        if (json.code !== 200) {
            throw new ClmmpoolsError(`Failed to get pool list from ${swapCountUrl}. Statu code is ${json.code}.`, RouterErrorCode.InvalidSwapCountUrl);
        }

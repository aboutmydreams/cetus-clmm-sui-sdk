...
import { CachedContent, asUintN, buildPosition, buildPositionReward, cacheTime24h, cacheTime5min, checkInvalidSuiAddress, extractStructTagFromType, getFutureTime, } from '../utils';
import { findAdjustCoin, TransactionUtil } from '../utils/transaction-util';
import { ClmmFetcherModule, ClmmIntegratePoolModule, ClmmIntegratePoolV2Module, ClmmIntegratePoolV3Module, CLOCK_ADDRESS, } from '../types/sui';
...
export class PositionModule {
    ...
    async createAddLiquidityFixTokenPayload(params, gasEstimateArg, tx, inputCoinA, inputCoinB) {
        ...
        if (gasEstimateArg) {
            ...
            if ((params.fix_amount_a && isAdjustCoinA) || (!params.fix_amount_a && isAdjustCoinB)) {
                tx = await TransactionUtil.buildAddLiquidityFixTokenForGas(this._sdk, allCoinAsset, params, gasEstimateArg, tx, inputCoinA, inputCoinB);
                return tx;
            }
        }
    ...
    async createAddLiquidityPayload(params, tx, inputCoinA, inputCoinB) {
        ...
        if (needOpenPosition) {
            ...
            const allCoinAsset = await this._sdk.getOwnerCoinAssets(this._sdk.senderAddress);
            tx = TransactionUtil.createCollectRewarderAndFeeParams(this._sdk, tx, params, allCoinAsset, primaryCoinAInputs.remainCoins, primaryCoinBInputs.remainCoins);
            tx.moveCall({

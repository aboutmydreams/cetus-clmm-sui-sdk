...
function getFeeInTickRange(clmmpool, tickLower, tickUpper) {
    ...
    const fee_growth_inside_a = MathUtil.subUnderflowU128(MathUtil.subUnderflowU128(new BN(clmmpool.fee_growth_global_a), fee_growth_below_a), fee_growth_above_a);
    const fee_growth_inside_b = MathUtil.subUnderflowU128(MathUtil.subUnderflowU128(new BN(clmmpool.fee_growth_global_b), fee_growth_below_b), fee_growth_above_b);
 ...
 */
export function collectFeesQuote(param) {

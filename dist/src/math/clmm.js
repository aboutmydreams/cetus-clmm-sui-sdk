...
export function toCoinAmount(a, b) {
    return {
 ...
 * @returns
 */
export function getDeltaA(sqrtPrice0, sqrtPrice1, liquidity, roundUp) {
 ...
 * @returns
 */
export function getDeltaB(sqrtPrice0, sqrtPrice1, liquidity, roundUp) {
    const sqrtPriceDiff = sqrtPrice0.gt(sqrtPrice1) ? sqrtPrice0.sub(sqrtPrice1) : sqrtPrice1.sub(sqrtPrice0);
 ...
 */
export function getNextSqrtPriceAUp(sqrtPrice, liquidity, amount, byAmountIn) {
    if (amount.eq(ZERO)) {
        return sqrtPrice;
    }
 ...
 * @param byAmountIn - Weather to fixed input
 */
export function getNextSqrtPriceBDown(sqrtPrice, liquidity, amount, byAmountIn) {
 ...
 * @returns
 */
export function getNextSqrtPriceFromInput(sqrtPrice, liquidity, amount, aToB) {
 ...
 * @returns
 */
export function getNextSqrtPriceFromOutput(sqrtPrice, liquidity, amount, a2b) {
 ...
 * @returns
 */
export function getDeltaUpFromInput(currentSqrtPrice, targetSqrtPrice, liquidity, a2b) {
 ...
 * @returns
 */
export function getDeltaDownFromOutput(currentSqrtPrice, targetSqrtPrice, liquidity, a2b) {
    const sqrtPriceDiff = currentSqrtPrice.gt(targetSqrtPrice) ? currentSqrtPrice.sub(targetSqrtPrice) : targetSqrtPrice.sub(currentSqrtPrice);
    if (liquidity.lte(ZERO) || sqrtPriceDiff.eq(ZERO)) {
        return ZERO;
    }
 ...
 * @returns
 */
export function computeSwapStep(currentSqrtPrice, targetSqrtPrice, liquidity, amount, feeRate, byAmountIn) {
    if (liquidity === ZERO) {
        return {
            amountIn: ZERO,
            amountOut: ZERO,
    ...
    let feeAmount;
    if (byAmountIn) {
        const amountRemain = MathUtil.checkMulDivFloor(amount, MathUtil.checkUnsignedSub(FEE_RATE_DENOMINATOR, feeRate), FEE_RATE_DENOMINATOR, 64);
 ...
 * @returns
 */
export function computeSwap(aToB, byAmountIn, amount, poolData, swapTicks) {
 ...
 * @return
 */
export function estimateLiquidityForCoinA(sqrtPriceX, sqrtPriceY, coinAmount) {
 ...
 * @return
 */
export function estimateLiquidityForCoinB(sqrtPriceX, sqrtPriceY, coinAmount) {
    const lowerSqrtPriceX64 = BN.min(sqrtPriceX, sqrtPriceY);
    const upperSqrtPriceX64 = BN.max(sqrtPriceX, sqrtPriceY);
...
export class ClmmPoolUtil {
    ...
    static calculateDepositRatioFixTokenA(lowerTick, upperTick, curSqrtPrice) {
        ...
        const { coinAmountB } = ClmmPoolUtil.estLiquidityAndcoinAmountFromOneAmounts(lowerTick, upperTick, coinAmountA, true, true, 0, curSqrtPrice);
